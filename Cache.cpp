...
bool Cache::validate(int server_fd,ClientRequest* request, ServerResponse* response){
	//need implementation
...
bool Cache::check_expire(int id,std::string info,ServerResponse* response){
	std::map<std::string, std::string> info_map=response->response_info;
...
void Cache::check_response_save(int server_fd,ClientRequest* request, ServerResponse* response ){
	...
	bool has_expire=check_expire(req_id,": cached, expires at",cresponse);
	...
	if(a||b){
		if(validate(server_fd,request,response)==false){
				this->cache_map.insert(this->cache_map.begin(),std::pair<std::string, std::string>(request_line,response->whole_response));
		}
	}else{
		this->cache_map.insert(this->cache_map.begin(),std::pair<std::string, std::string>(request_line,response->whole_response));
